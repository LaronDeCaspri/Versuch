/*
 * Jarvis — German in-browser voice assistant. Web Speech API only, zero cloud.
 * Listens to `window.dispatchEvent(new CustomEvent('jarvis', {detail:{text,level}}))`
 * events. Use jarvis.speak(text, level) to speak directly.
 */
(function () {
    const state = {
    ...
    function speak(text, level = "info") {
        if (!text) return;
        writeToLog({ ts: new Date().toISOString(), text, level });
    ...
    function updateUI() {
        ...
        if (badge) badge.textContent = state.enabled ? "aktiv" : "deaktiviert";
    }
    ...
    window.jarvis = { speak, enable, disable };

    window.addEventListener("jarvis", (ev) => {
        speak(ev.detail?.text || "", ev.detail?.level || "info");
    });
